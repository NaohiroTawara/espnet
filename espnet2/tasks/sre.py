...
pooling_choices = ClassChoices(
    "pooling",
    classes=dict(
        max=GlobalMaxPooling, tap=GlobalAveragePooling, sap=SelfAttentionPooling, stat=StatisticalPooling
    ),
...
class SRETask(AbsTask):
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            "--max_seg_per_spk", type=int, default=100,
            help="The maximum number of segments per speaker in each epoch",
        )
        group.add_argument(
            "--embed_size",
            type=int,
