...
sre_config=conf/train_sre_resnet34_softmax.yaml # Config for sre model training.
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if ! "$skip_dumping" ; then
            ...
            log "Stage 3: Format wav.scp is Skipped and use original $data_feats"
        fi
    fi
...
if ! "${skip_train}"; then
    if "${collect_feats}"; then
        if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
            ...
            ${train_cmd} JOB=1:"${nj}" "${sre_exp}"/stats.JOB.log \
           ...
           log "Skip the collecting stats for training"
        fi
        train_file="feats.scp,speech,npy"
    else
        train_file="wav.scp,speech,sound"
    fi

    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        log "Stage 6: SRE Training: train_set=${data_feats}/${train_set}, valid_set=${data_feats}/${valid_set}"
        ...
        if [ "${num_splits}" -gt 1 ]; then
            ...
            if [ ! -f "${_split_dir}/.done" ]; then
                ...
                ${python} -m espnet2.bin.split_scps \
                  --scps "${data_feats}/${train_set}/$train_file" \
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/${train_set}/${train_file} "
        ...
        mkdir -p "${sre_exp}"; echo "${run_args} --stage 6 \"\$@\"; exit \$?" > "${sre_exp}/run.sh"; chmod +x "${sre_exp}/run.sh"
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        log "Stage 7: Decoding: training_dir=${sre_exp}"
        ...
        # 1. Generate run.sh
        log "Generate '${sre_exp}/${inference_tag}/run.sh'. You can resume the process from stage 7 using this script"
        mkdir -p "${sre_exp}/${inference_tag}"; echo "${run_args} --stage 7 \"\$@\"; exit \$?" > "${sre_exp}/${inference_tag}/run.sh"; chmod +x "${sre_exp}/${inference_tag}/run.sh"
